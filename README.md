...
**✱** `next-multilingual` only works with 📁 `pages` directory. We are still ironing out our solution to support the new 📁 `app` directory since [internationalization](https://nextjs.org/docs/app/building-your-application/routing/internationalization) is no longer part of Next.js' config.
